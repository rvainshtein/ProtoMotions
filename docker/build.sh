...
docker run --name temp_container --gpus=all -e "ACCEPT_EULA=Y" -v ~/dev/MaskedMimic:/home/gymuser/MaskedMimic --ipc=host "${image_name}" bash -c "docker/local_package_installs.sh"
# This is here because the data_gen_invocation_script.sh is not able to run in the Dockerfile
docker run --name temp_container --gpus=all -e "ACCEPT_EULA=Y" -v ~/dev/MaskedMimic:/home/gymuser/MaskedMimic --ipc=host "${image_name}" bash -c "docker/data_gen_invocation_script.sh"
